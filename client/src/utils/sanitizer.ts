...
function name(name: string): string {
    if (!name.length) return 'is missing';
    else if (name.length < 3) return 'is too short, needs at least 3 characters';
...
function email(email: string): string {
    ...
    if (!email.length) return 'is missing'
    else if (!regex.test(email)) return 'is bad formatted';
...
function password(password: string): string {
    ...
    if (!password.length) password_warnings.push('is missing');
    else {
...
function confirm_password(password: string, confirm_password: string): string {
