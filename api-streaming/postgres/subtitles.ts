...
export async function getTorrentSubtitles(torrentId: number) {
	const res = await client.query<Subtitle>("SELECT * FROM torrent_subtitles WHERE torrent_id = $1", [torrentId]);
	return res.rows;
}
...
export async function getMediaInformations(mediaId: number) {
	const media = await client.query<MediaInformations>("SELECT imdb_id, tmdb_id, year FROM medias WHERE id = $1", [
		mediaId,
	]);
...
export function createTorrentSubtitle(informations: { torrent_id: number; lang: string; path: string }) {
