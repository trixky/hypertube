...
export function read_env() {
    ...
    if (isNaN(config.REDIS_port))
        throw new Error('REDIS_PORT environment variable is missing')
    // ----------------------------------- CLIENT
    config.CLIENT_port = parseInt(process.env.CLIENT_PORT === undefined ? 'x' : process.env.CLIENT_PORT);
    if (isNaN(config.CLIENT_port))
        throw new Error('CLIENT_PORT environment variable is missing')
    // ----------------------------------- API_AUTH
    config.API_AUTH_port = parseInt(process.env.API_AUTH_GRPC_GATEWAY_PORT === undefined ? 'x' : process.env.API_AUTH_GRPC_GATEWAY_PORT);
    if (isNaN(config.API_AUTH_port))
        throw new Error('API_AUTH_GRPC_GATEWAY_PORT environment variable is missing')
    // ----------------------------------- API_USER
    config.API_USER_port = parseInt(process.env.API_USER_GRPC_GATEWAY_PORT === undefined ? 'x' : process.env.API_USER_GRPC_GATEWAY_PORT);
    if (isNaN(config.API_USER_port))
        throw new Error('API_USER_GRPC_GATEWAY_PORT environment variable is missing')
    // ----------------------------------- API_POSITION
    config.API_POSITION_port = parseInt(process.env.API_POSITION_PORT === undefined ? 'x' : process.env.API_POSITION_PORT);
