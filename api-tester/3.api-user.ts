...
const API_USER_request = supertest(`${get_config().API_USER_addresse}:${get_config().API_USER_port}`);

export function DESCRIBE_TEST_api_user() {
    // ---------------------------------------- GET 404 /v1/me (token missing)
    it("GET 404 /v1/me (token missing)", (done) => {
        API_USER_request
            .get("/v1/me" + shared.random_user_token)
            .send()
            .expect(404)
            .end((err: any) => {
    ...
    it("GET 400 /v1/me (token corrupted)", (done) => {
        API_USER_request
            .get("/v1/me?token=" + shared.random_user_token_corrupted)
            .send()
            .expect(400)
            .end((err: any) => {
                if (err) return done(err);
    ...
    it("GET 200 /me", (done) => {
        API_USER_request
            .get("/v1/me?token=" + shared.random_user_token)
            .send()
            .expect(200)
            .end((err: any) => {
                if (err) return done(err);
    ...
    it("PATCH 400 /v1/me (corrupted email)", (done) => {
        ...
        API_USER_request
            .patch("/v1/me" + "?" + url_search_params.toString())
    ...
    it("GET 200 /v1/user", (done) => {
        API_USER_request
            .get("/v1/user?id=" + shared.random_user_userInfo!.id)
            .set("Cookie", `token=${shared.random_user_token};`)
            .send()
            .expect(200)
            .expect(res => {
                if (res?.body?.userInfo == undefined)
                    throw new Error("userinfo missing")
            })
            .end((err: any) => {
                if (err) return done(err);
                return done();
